...
var ace = require("./proto/ace");
var agent = require("./proto/agent");
var base = require("./proto/base");
var querys = require("./proto/querys");
var queues = require("./proto/queues");
var stripe = require("./proto/stripe");
var watch = require("./proto/watch");
var workitems = require("./proto/workitems");
var protowrap = /** @class */ (function () {
    ...
    protowrap.init = function () {
        return __awaiter(this, void 0, void 0, function () {
            var paths, basepath, basepath, packageDefinition;
    ...
    //   }
    // }
    protowrap.lookupType = function (command) {
        ...
        switch (command) {
            ...
            default:
                ...
                var idx = key.map(function (k) { return k.toLowerCase(); }).indexOf(searchkey);
                if (idx > -1)
                    return ace[key[idx]];
                key = Object.keys(agent);
                idx = key.map(function (k) { return k.toLowerCase(); }).indexOf(searchkey);
                if (idx > -1)
                    return agent[key[idx]];
                ...
                idx = key.map(function (k) { return k.toLowerCase(); }).indexOf(searchkey);
                if (idx > -1)
                    return querys[key[idx]];
                key = Object.keys(queues);
                idx = key.map(function (k) { return k.toLowerCase(); }).indexOf(searchkey);
                if (idx > -1)
                    return queues[key[idx]];
                key = Object.keys(stripe);
                idx = key.map(function (k) { return k.toLowerCase(); }).indexOf(searchkey);
                if (idx > -1)
                    return stripe[key[idx]];
                key = Object.keys(watch);
                idx = key.map(function (k) { return k.toLowerCase(); }).indexOf(searchkey);
                if (idx > -1)
                    return watch[key[idx]];
                key = Object.keys(workitems);
                idx = key.map(function (k) { return k.toLowerCase(); }).indexOf(searchkey);
                if (idx > -1)
                    return workitems[key[idx]];
        }
        return null;
    };
    ...
    protowrap.pack = function (command, payload) {
        ...
        var protomsg = this.lookupType(command);
        if (protomsg == null)
            return null;
    ...
    protowrap.DownloadFile = function (client, id, filename, folder, highWaterMark) {
        var _this = this;
        return new Promise(function (resolve, reject) { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                ...
                packdata = any_1.Any.create({ type_url: "type.googleapis.com/openiap.DownloadRequest", value: base_1.DownloadRequest.encode(base_1.DownloadRequest.create({ id: id, filename: filename })).finish() });
    ...
    protowrap.UploadFile = function (client, filename, jwt) {
        var basename = path.basename(filename);
        var packdata = any_1.Any.create({ "typeUrl": "openiap.UploadRequest", "value": base_1.UploadRequest.encode(base_1.UploadRequest.create({ filename: basename })).finish() });
